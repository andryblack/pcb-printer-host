...
namespace clipperlib {
  ...
  static inline bool IntersectListSort(IntersectNode *node1, IntersectNode *node2)
  {
  ...
  Clipper::Clipper() : actives_(0),sel_(0)
  {
    Clear();
  }
  //--------------------------- ---------------------------------------------------
  ...
  void Clipper::AddPathToVertexList(const Path &path, PathType polytype, bool is_open) 
  {
    ...
    vertex_list_.push_back(VerticesVector());
    VerticesVector& vertices(vertex_list_.back());
    vertices.resize(path_len);
  ...
  void Clipper::AddLocalMinPoly(Active &e1, Active &e2, const Point64 pt)
  {
    ...
    OutRec& outrec(outrec_list_.back());
    outrec.idx = (unsigned)outrec_list_.size()-1;
    ...
    for (size_t i = 0; i < cnt; ++i) {
      if (!EdgesAdjacentInSel(*intersect_list_[i])) {
        ...
        for (size_t j = i + 1; j < cnt; ++j) {
          if (EdgesAdjacentInSel(*intersect_list_[j])) {
